...
if (!role || !out) {
  console.error('usage: node remotion/render_flyer_v11.mjs <role> <out.png>');
  process.exit(2);
}

const props = {
  role,
  cta: process.env.CTA || 'Get Government\nContracts Free',
...
const cmd = `cd "${REM_DIR}" && npx remotion still src/index.ts StillFlyerV11 "${out}" --props='${JSON.stringify(props).replace(/'/g, "'\\''")}' --log=warn`;
