...
export const StillFlyerV2: React.FC<StillFlyerV2Props> = (p) => {
  const bg = 'linear-gradient(180deg, #f8fbff 0%, #eef6ff 60%, #eaf3ff 100%)';
  const blue = '#2563EB';
  ...
  return (
    <AbsoluteFill style={{ background: bg, fontFamily: 'ui-sans-serif, system-ui, -apple-system' }}>
      ...
      <div style={{ position: 'absolute', left: 60, right: 60, top: 54, height: 86, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Img src={staticFile('assets/maxcontrax/logo.png')} style={{ height: 54, width: 'auto' }} />
        <div style={{ fontSize: 16, fontWeight: 900, color: 'rgba(15,23,42,0.55)' }}>Official Contract Matches</div>
      </div>
      ...
      <div style={{ position: 'absolute', left: 60, top: 180, width: 600 }}>
        ...
        <div style={{ marginTop: 26, width: 560, borderRadius: 28, background: 'rgba(255,255,255,0.95)', border: '1px solid rgba(0,0,0,0.07)', boxShadow: '0 26px 70px rgba(0,0,0,0.18)', padding: 24 }}>
          ...
          <div style={{ marginTop: 18, position: 'relative' }}>
            <div style={{ position: 'absolute', left: 18, right: 18, top: 10, height: 62, borderRadius: 18, background: blue, opacity: 0.28, filter: 'blur(18px)' }} />
            <div style={{ position: 'relative', background: blue, borderRadius: 18, padding: '16px 18px', textAlign: 'center', color: '#fff', fontSize: 30, fontWeight: 950, letterSpacing: -0.2, boxShadow: '0 14px 30px rgba(0,0,0,0.14)' }}>
              {p.ctaText}
            </div>
          </div>
        </div>
      ...
      <div style={{ position: 'absolute', right: 70, bottom: 90, width: 420 }}>
        <div style={{ position: 'absolute', inset: -18, borderRadius: 42, background: 'radial-gradient(circle at 30% 30%, rgba(37,99,235,0.16), rgba(37,99,235,0.0) 70%)', filter: 'blur(14px)' }} />
