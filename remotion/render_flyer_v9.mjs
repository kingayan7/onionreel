...
if (!role || !out) {
  console.error('usage: node remotion/render_flyer_v9.mjs <role> <out.png>');
  process.exit(2);
}
...
const cmd = `cd "${REM_DIR}" && npx remotion still src/index.ts StillFlyerV9 "${out}" --props='${JSON.stringify(props).replace(/'/g, "'\\''")}' --log=warn`;
