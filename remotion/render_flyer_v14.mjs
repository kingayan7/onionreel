...
if (!out) {
  console.error('usage: node remotion/render_flyer_v14.mjs <out.png>');
  process.exit(2);
}
...
const cmd = `cd "${REM_DIR}" && npx remotion still src/index.ts StillFlyerV14 "${out}" --props='${JSON.stringify(props).replace(/'/g, "'\\''")}' --log=warn`;
